...
original_request_id_var = ContextVar('original_request_id', default='None')
...
class RequestIdFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        record.original_request_id = original_request_id_var.get()
        return True
...
def setup_logging():
    ...
    if format_choice == 'json':
        ...
        formatter = logging.Formatter(
            '[%(asctime)s] - [%(levelname)s] - [%(name)s] - [rid:%(request_id)s] [oid:%(original_request_id)s] - %(message)s'
        )
